# Tests for an unsuccessful signin
# -----------------------------------

# Given a user visits the signin page
Given /^a user visits the signin page$/ do
  visit signin_path
end

# When he submits invalid signin information
When /^he submits invalid signin information$/ do
  click_button "Sign in"
end

# Then he should see a bad login error message
Then /^he should see a bad login error message$/ do 
  page.should have_selector('div.alert.alert-error')
end

# Tests for a successful signin
# -----------------------------------

# And the user has an account
Given /^the user has an account$/ do
  @user =  User.create(name: "Bill Clinton", email: "willy@whitehouse.com", password: "foobar", password_confirmation: "foobar")
end

# And the user submits valid signin information
Given /^the user submits valid signin information$/ do
  fill_in "Email", with: @user.email
  fill_in "Password", with: @user.password  
  click_button "Sign in"
end

# Then he should see his profile page
Then /^he should see his profile page$/ do
  page.should have_selector('title', text: @user.name)
end

# And he should see a signout link
Then /^he should see a signout link$/ do
  page.should have_link('Sign Out', href: signout_path)
end



